...
public class JdbcSeatOverviewDAO implements SeatOverviewDAO {
	...
	@Override
	public List<TrainConnection> getTrainConnections(String start, String destination, Long departureTime, Integer day,
			boolean departure, Long timeTillDay) {
		...
		try {
			...
			/*
			 * Je nachdem, ob departure wahr oder falsch ist, beziehen wir uns
			 * auf die Ankunfts- oder Abfahrtszeit und laden andere Queries. Das
			 * hier ist die Queries um alle passenden Zugfahrten zu wählen
			 * 
			 * TODO: Die Queries da zusammenfasse, wo es geht
			 */
			if (departure) {
				/*
				 * Die Queries liegen in den Resourcen und werden hier
				 * ausgelesen.
				 */
				getTrainConnections = IOUtils
						.toString(classLoader.getResourceAsStream("Queries/get-train-connections-departure"), "UTF-8");
			} else {
				getTrainConnections = IOUtils
						.toString(classLoader.getResourceAsStream("Queries/get-train-connections-arrival"), "UTF-8");
			}
			/*
			 * Query für die Zusatzinformationen (Zugnummer, Kategorie)
			 */
			String getTrainInformation = IOUtils
					.toString(classLoader.getResourceAsStream("Queries/get-additional-information"), "UTF-8");

			/*
			 * Query für die Zwischenhalte
			 */
			String getIntermediateStations = IOUtils
					.toString(classLoader.getResourceAsStream("Queries/get-intermediate-stations"), "UTF-8");
			...
			/*
			 * Das Statement braucht die Parameter. Die 120 beziehen sich
			 * darauf, dass alle passenden Fahrten in einem Zeitabschnitt von 2
			 * Stunden nach der Wunschzeit angezeigt werden
			 */
			...
			System.out.println("Executing Query: " + getTrainConnections);
			...
			while (trainConnectionRS.next()) {
				...
				/*
				 * Für jede dieser Zugfahrten müssen noch die
				 * Zusatzinformationen gesammelt werden
				 */
				System.out.println("Executing Query: " + getTrainInformation + " and " + getIntermediateStations
						+ "for id " + trainConnectionRS.getInt("id"));
				...
				/*
				 * next ist immer nötig, damit man in das ResultSet rein kommt
				 * (also in die erste Zeile)
				 */
				...
				while (intermediateStationsRS.next()) {
					...
					intermediateStations.add(new IntermediateStation(intermediateStationsRS.getString("station_name"),
							SeatOverviewService.dateTimeFormat.format(stopTime)));
				}

				/*
				 * Auf Grundlage dessen werden dann die einzelnen Ergebnisse
				 * zusammengesetzt und ganz am Ende zurück gegeben
				 */
				...
				trainConnections.add(new TrainConnection(trainConnectionRS.getInt("id"),
						trainInformationRS.getInt("number"), trainInformationRS.getString("category"),
						SeatOverviewService.dateTimeFormat.format(departureDate), SeatOverviewService.dateTimeFormat.format(arrivalDate), intermediateStations));
	...
	public List<Seat> getSeat(Integer trainConnectionID, Integer seatClass, String area, String location,
		...
		/*
		 * Wir wollen ja immer einen Platz ausgeben, sofern es genug Plätze
		 * gibt. Deswegen ruft getSeat einfach getAllSeats auf und gibt dann nur
		 * eine Teilmenge zurück
		 * 
		 * Die Teilmenge umfasst, wie viele Plätze gebucht werden sollten
		 * (numberOfPersons)
		 * 
		 * An dieser Stelle bräuchte es ggf. Sonderlogik, nach der wir die
		 * möglichst passenden Plätze auswählen
		 * 
		 * Wenn wir direkt nach den Eigenschaften fragen würden, würden wir aber
		 * potentiell keinen passenden Platz finden, müssten dann noch eine
		 * Anfrage mit weniger Eigenschaften stellen usw. und das ist viel
		 * Traffic hin und her. Daher das TODO
		 */
		List<Seat> seats = this.getAllSeats(trainConnectionID, departure, arrival);
		...
		/*
		 * wenn count nie so groß wie numberOfPerson wurde, gibt es nicht genug
		 * Plätze und wir geben null zurück
		 */
		return null;
	...
	public List<Seat> getAllSeats(int trainConnectionID, Long departure, Long arrival) {
		...
		try {
			 ...
			 * 
			 * Was sie tut ist: 1. Suche alle Plätze, die zur Zugfahrt passen 2.
			 * Entfernt die Plätze, die für diesen Tag und diese Zeit bereits
			 * belegt sind (umfasst auch Plätze, die nur zwischendrin oder
			 * abschnittsweise eine Überschneidung haben)
			 */
			String getSeats = IOUtils.toString(classLoader.getResourceAsStream("Queries/get-available-seats"), "UTF-8");
			...
			System.out.print("Executing query " + getSeats);
			...
			/*
			 * Für jede zulässige ID werden dann die anderen Informationen zum
			 * Platz gesucht und ein neues Objekt erstellt
			 */
			while (seatsRS.next()) {
				...
				if (seat.next()) {
					seats.add(new Seat(seat.getInt("id"), seat.getInt("class"), seat.getString("area"),
							seat.getString("location"), seat.getString("compartment_type"),
							seat.getBoolean("upper_level"), seat.getInt("wagon_id")));
				}
